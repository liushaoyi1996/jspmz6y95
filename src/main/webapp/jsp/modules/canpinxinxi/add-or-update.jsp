...
<body>
	<div id="main-container">
		...
		<div id="breadcrumb-container">
			<h3 class="breadcrumb-title">编辑餐品信息</h3>
			<ol class="breadcrumb-list">
				...
				<li class="breadcrumb-item-one"><span>餐品信息管理</span></li>
				<li class="breadcrumb-item-two"><span>编辑餐品信息</span></li>
			</ol>
		</div>
		...
		<div id="add-container">
			<form id="addOrUpdateForm">
				...
				<div class="form-item">
					<label>餐品分类</label>
					<select id="canpinfenleiSelect" name="canpinfenlei" class="form-control">
							<option value="">请选择餐品分类</option>
					</select>	
				</div>	
				...
				<div class="form-item-file" >
					...
					<div class="upload btn-file">
						插入图片
						<input id="canpinxiangqingupload" name="file" type="file">
					</div>
					<script id="canpinxiangqingEditor" type="text/plain"></script>
					<script type="text/javascript">
						//建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
					   //相见文档配置属于你自己的编译器
					 var canpinxiangqingUe = UE.getEditor('canpinxiangqingEditor', {
						toolbars: [
							[
								'undo', //撤销
								'bold', //加粗
					...
					<input type="hidden" name="canpinxiangqing" id="canpinxiangqing-input">
				</div>
				<div class="form-btn">
	...
	<script>
		...
		var canpinfenleiOptions = [];


		function canpinfenleiSelect(){
			http("option/canpinfenlei/canpinfenlei","GET",{},(res)=>{
				if(res.code == 0){
					canpinfenleiOptions = res.data;
					var canpinfenleiSelect = document.getElementById('canpinfenleiSelect');
					for(var i=0;i<canpinfenleiOptions.length;i++){
						var canpinfenleiOption = document.createElement('option');
						canpinfenleiOption.setAttribute('name','canpinfenleiOption');
						canpinfenleiOption.setAttribute('value',canpinfenleiOptions[i]);
						canpinfenleiOption.innerHTML = canpinfenleiOptions[i];									
						if(ruleForm.canpinfenlei == canpinfenleiOptions[i]){
							canpinfenleiOption.setAttribute('selected','selected');
						}
						canpinfenleiSelect.appendChild(canpinfenleiOption);
					}
				}
			});
		}	
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "canpinxinxi/info/" + id,
		...
		function upload(){
			...
			$('#canpinxiangqingupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {	
					UE.getEditor('canpinxiangqingEditor').execCommand('insertHtml','<img src=\"'+ baseUrl +'upload/'+ data.result.file + '\" width=900 height=560>');					
				}
			});
		}  
		//取消 
				...
				if(crossrefid && crossuserid) {
					...
					http("canpinxinxi/page","GET",{
						...
						if(res.data.total >= crossoptnum){
							...
							httpJson("canpinxinxi/"+urlParam,"POST",data,(res)=>{
								if(res.code == 0){
		...
		// 填充富文本框
		function setContent(){
			if(ruleForm.canpinxiangqing != null && ruleForm.canpinxiangqing != 'null' && ruleForm.canpinxiangqing != ''){
				var canpinxiangqingMes = '' + ruleForm.canpinxiangqing;
				var canpinxiangqingUeditor = UE.getEditor('canpinxiangqingEditor');
				canpinxiangqingUeditor.ready(function() {
					canpinxiangqingUeditor.setContent(canpinxiangqingMes);
				});
			}
		}  
		...
		// 获取富文本框内容
		function getContent(){
			if(UE.getEditor('canpinxiangqingEditor').hasContents()){
				$('#canpinxiangqing-input').attr('value',UE.getEditor('canpinxiangqingEditor').getContent());
			}
		}
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("canpinxinxi/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function dataBind(){
			...
			var canpinfenleiOptions = document.getElementsByClassName("canpinfenleiOption");
			for(var canpinfenleiCount = 0; canpinfenleiCount < canpinfenleiOptions.length;canpinfenleiCount++){
				if(canpinfenleiOptions[canpinfenleiCount].getAttribute('value') == ruleForm.canpinfenlei){
					canpinfenleiOptions[canpinfenleiCount].setAttribute('selected','selected');
				}
			}
            if(ruleForm.tupian){
			...
			$("#canpinxiangqing").val(ruleForm.canpinxiangqing);	
		}		
		//图片显示
		function showImg(){
			...
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.canpinfenlei != null && res.data.canpinfenlei != ''){
						...
						var canpinfenleiOptions = document.getElementsByClassName("canpinfenleiOption");
						for(var canpinfenleiCount = 0; canpinfenleiCount < canpinfenleiOptions.length;canpinfenleiCount++){
							if(canpinfenleiOptions[canpinfenleiCount].getAttribute('value') == res.data.canpinfenlei){
								canpinfenleiOptions[canpinfenleiCount].setAttribute('selected','selected');
							}
						}
					}
					...
					if(res.data != null && res.data != '' && res.data.canpinxiangqing != null && res.data.canpinxiangqing != ''){

						$("#canpinxiangqing").val(res.data.canpinxiangqing);
                        var canpinxiangqingMes = '' + res.data.canpinxiangqing;
                        var canpinxiangqingUeditor = UE.getEditor('canpinxiangqingEditor');
                        canpinxiangqingUeditor.ready(function() {
                            canpinxiangqingUeditor.setContent(canpinxiangqingMes);
                        });
