...
<body>
	<div id="main-container">
		...
		<div id="breadcrumb-container">
			<h3 class="breadcrumb-title">编辑用户</h3>
			<ol class="breadcrumb-list">
				...
				<li class="breadcrumb-item-one"><span>用户管理</span></li>
				<li class="breadcrumb-item-two"><span>编辑用户</span></li>
			</ol>
		</div>
		...
		<div id="add-container">
			<form id="addOrUpdateForm">
				...
				<div class="form-item">
					<label>会员等级</label>
					<select  class="form-control"  name="huiyuandengji" id="huiyuandengjiSelect" onchange="huiyuandengjiSelect2Change()">
					</select>
				</div>
				<div class="form-item">
					<label>会员折扣</label>
					<input id="huiyuanzhekou" name="huiyuanzhekou" class="form-control"  value="" readonly>
				</div>
	...
	<script>
		...
		function huiyuandengjiSelect2(){
			http("option/huiyuandengji/huiyuandengji","GET",{
				tableName: "huiyuandengji",
				columnName: "huiyuandengji"
			},(res)=>{
				if(res.code == 0){
					var options = res.data;
					var option = document.createElement("option");
                    option.innerHTML = "请选择会员等级";
					$("#huiyuandengjiSelect").append(option)
					for(var i=0;i<options.length;i++){
						var huiyuandengjiOption = document.createElement("option");
						huiyuandengjiOption.setAttribute("value",options[i]);
						huiyuandengjiOption.setAttribute("class","huiyuandengjiOption");
						if(ruleForm.huiyuandengji == options[i]){
							huiyuandengjiOption.setAttribute("selected","selected");
						}
						huiyuandengjiOption.innerHTML = options[i];
						//修改 回显
						if(ruleForm.huiyuandengji != null && ruleForm.huiyuandengji != '' && options[i] == ruleForm.huiyuandengji){
							huiyuandengjiOption.setAttribute("selected","selected");
						}
						$("#huiyuandengjiSelect").append(huiyuandengjiOption);
					}
				}
			});
		}		
		function huiyuandengjiSelect2Change(){
			var options = document.getElementById("huiyuandengjiSelect").options;
			var index = document.getElementById("huiyuandengjiSelect").selectedIndex;
			var colVal = options[index].value;
			http("follow/huiyuandengji/huiyuandengji","GET",{
				tableName: "huiyuandengji",
				columnName: "huiyuandengji",
				columnValue: colVal
			},(res)=>{
				if(res.code == 0){
					if(res.data != null && res.data.huiyuanzhekou != null){
		...
		function submit() {
                //获取表单数据前取消禁用
                $("#huiyuandengjiSelect").attr('disabled',false);
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				...
				var huiyuandengjiOptions = document.getElementsByClassName("huiyuandengjiOption");
				for(var huiyuandengjiCount = 0; huiyuandengjiCount < huiyuandengjiOptions.length;huiyuandengjiCount++){
					if(huiyuandengjiOptions[huiyuandengjiCount].getAttribute('value') == "会员等级1"){
						huiyuandengjiOptions[huiyuandengjiCount].setAttribute('selected','selected');
					}
				}
				$("#huiyuanzhekou").val("1");
		...
		function readonly(){
            if(window.sessionStorage.getItem('role')!="管理员") {
                ...
                $("#huiyuandengjiSelect").attr('disabled',true);
                $("#huiyuanzhekou").attr('readonly','readonly');
            }
		}
