...
<body>
	<div id="main-container">
		...
		<div id="breadcrumb-container">
			<h3 class="breadcrumb-title">包间预订管理</h3>
			<ol class="breadcrumb-list">
				...
				<li class="breadcrumb-item-one"><span>包间预订管理</span></li>
				<li class="breadcrumb-item-two"><span>包间预订列表</span></li>
			</ol>
		</div>
		<!-- /Breadcrumb -->
		...
		<div id="center-container">
			...
			<div class="center-form">
				...
				<div class="form-item">
					<label>用户账号</label>
					<input type="text" id="yonghuzhanghaoSearch" class="form-control form-control-sm" placeholder="请输入用户账号" aria-controls="tableId">
				</div>
				<div class="form-item">
					<label>审核状态</label>
			...
			<div class="btns">
				...
				<button onclick="chartDialog1()" type="button" class="btn btn-info btn-statis" v-if="crossBtnControl('包间收入统计')"><i class="fa fa-database"></i><span>包间收入统计</span></button>
			</div>
			...
			<table id="tableId" class="table table-bordered">
				<thead>
					<tr>
						...
						<th onclick="sort('huiyuandengji')">会员等级<i id="huiyuandengjiIcon" class="fa fa-sort"></i></th>
						<th onclick="sort('huiyuanzhekou')">会员折扣<i id="huiyuanzhekouIcon" class="fa fa-sort"></i></th>
						<th onclick="sort('zhekoujiage')">折扣价格<i id="zhekoujiageIcon" class="fa fa-sort"></i></th>
						<th onclick="sort('yudingshijian')">预订时间<i id="yudingshijianIcon" class="fa fa-sort"></i></th>
            ...
            <div class="openBox" v-if="showType1">
                ...
                <div class="openView">
                    <div id="zhekoujiageMain1" class="graph" style="width: 70vw;height:50vh;"></div>
                </div>
            </div>
		</div>
	...
	<script>
		...
		function setDataRow(item,number){
			...
			var huiyuandengjiCell  = document.createElement('td');

						 huiyuandengjiCell.innerHTML = item.huiyuandengji;

			row.appendChild(huiyuandengjiCell);
			var huiyuanzhekouCell  = document.createElement('td');

						 huiyuanzhekouCell.innerHTML = item.huiyuanzhekou;

			row.appendChild(huiyuanzhekouCell);
			var zhekoujiageCell  = document.createElement('td');

						 zhekoujiageCell.innerHTML = item.zhekoujiage;

			row.appendChild(zhekoujiageCell);
		...
		function shDialog(id){
			$("#shId").attr("value",id);
			http("baojianyuding/info/" + id,"GET",{},(res)=>{
				if(res.code == 0){
		...
		function sh(){
			...
			httpJson("baojianyuding/update","POST",data,(res)=>{
				if(res.code == 0){
					alert("提交成功");
					getDataList();
				}
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("baojianyuding/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function getSum(colName){
		http("baojianyuding"+colName,"GET",{
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"baojianyuding");
        ...
        function pay(item){
			...
			window.sessionStorage.setItem('tableName',"baojianyuding");
                        ...
                        success: function(data){
                            if(data.code == 0){
                                if(data.data != null){
                                    ...
                                    var myChart = echarts.init(document.getElementById('zhekoujiageMain1'),'green');
                                    var option = {};
                                    option = {
                                        title: {
                                            text: '包间收入统计',
