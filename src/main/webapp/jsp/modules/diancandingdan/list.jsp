...
<body>
	<div id="main-container">
		...
		<div id="breadcrumb-container">
			<h3 class="breadcrumb-title">点餐订单管理</h3>
			<ol class="breadcrumb-list">
				...
				<li class="breadcrumb-item-two"><span>点餐订单列表</span></li>
			</ol>
		</div>
		<!-- /Breadcrumb -->
		...
		<div id="center-container">
			...
			<div class="center-form">
				<div class="form-item">
					<label>餐品名称</label>
					<input type="text" id="canpinmingchengSearch" class="form-control form-control-sm" placeholder="请输入餐品名称" aria-controls="tableId">
				</div>
				<div class="form-item">
					<label>审核状态</label>
			...
			<div class="btns">
				...
				<button onclick="chartDialog1()" type="button" class="btn btn-info btn-statis" v-if="crossBtnControl('点餐收入统计')"><i class="fa fa-database"></i><span>点餐收入统计</span></button>
			</div>
			...
			<table id="tableId" class="table table-bordered">
				<thead>
					<tr>
						...
						<th onclick="sort('canpinmingcheng')">餐品名称<i id="canpinmingchengIcon" class="fa fa-sort"></i></th>
						<th onclick="sort('canpinfenlei')">餐品分类<i id="canpinfenleiIcon" class="fa fa-sort"></i></th>
						...
						<th onclick="sort('baojianhao')">包间号<i id="baojianhaoIcon" class="fa fa-sort"></i></th>
						<th onclick="sort('dingcanbeizhu')">订餐备注<i id="dingcanbeizhuIcon" class="fa fa-sort"></i></th>
						<th onclick="sort('xiadanshijian')">下单时间<i id="xiadanshijianIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function search(){
			searchForm = { key: ""};
			if($('#canpinmingchengSearch').val() != null && $('#canpinmingchengSearch').val() != ''){
				searchForm.canpinmingcheng ="%" +  $('#canpinmingchengSearch').val() + "%";
			}
        ...
        function getDataList() {
			http("diancandingdan/page","GET",{
				...
				sort: sortColumn,
				order: sortOrder,
				canpinmingcheng : searchForm.canpinmingcheng,	
			},(res)=>{
				if(res.code == 0){
					clear();
		...
		function setDataRow(item,number){
			...
			var canpinmingchengCell  = document.createElement('td');

						 canpinmingchengCell.innerHTML = item.canpinmingcheng;

			row.appendChild(canpinmingchengCell);
			var canpinfenleiCell  = document.createElement('td');

						 canpinfenleiCell.innerHTML = item.canpinfenlei;

			row.appendChild(canpinfenleiCell);
			...
			var baojianhaoCell  = document.createElement('td');

						 baojianhaoCell.innerHTML = item.baojianhao;

			row.appendChild(baojianhaoCell);
			var dingcanbeizhuCell  = document.createElement('td');

						 dingcanbeizhuCell.innerHTML = item.dingcanbeizhu;

			row.appendChild(dingcanbeizhuCell);
			var xiadanshijianCell  = document.createElement('td');

						  xiadanshijianCell.innerHTML = item.xiadanshijian;


			row.appendChild(xiadanshijianCell);
		...
		function sh(){
			...
			httpJson("diancandingdan/update","POST",data,(res)=>{
				if(res.code == 0){
					alert("提交成功");
					getDataList();
				}
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("diancandingdan/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"diancandingdan");
        ...
        function pay(item){
			...
			window.sessionStorage.setItem('tableName',"diancandingdan");
                        ...
                        success: function(data){
                            if(data.code == 0){
                                if(data.data != null){
                                    ...
                                    var myChart = echarts.init(document.getElementById('zhekoujiageMain1'),'carp');
                                    var option = {};
                                    option = {
                                        title: {
                                            text: '点餐收入统计',
